...
internal sealed class Scanner {
	...
	static Scanner() {
		...
		start[34] = 21; 
		start[60] = 59; 
		for (int i = 38; i <= 38; ++i) start[i] = 35;
		for (int i = 61; i <= 61; ++i) start[i] = 35;
		for (int i = 124; i <= 124; ++i) start[i] = 35;
		start[62] = 60; 
		start[94] = 38; 
		start[58] = 61; 
		start[43] = 40; 
		start[45] = 62; 
		for (int i = 37; i <= 37; ++i) start[i] = 42;
		for (int i = 42; i <= 42; ++i) start[i] = 42;
		for (int i = 47; i <= 47; ++i) start[i] = 42;
		for (int i = 33; i <= 33; ++i) start[i] = 45;
		for (int i = 63; i <= 63; ++i) start[i] = 45;
		for (int i = 126; i <= 126; ++i) start[i] = 45;
		start[123] = 49; 
		start[125] = 50; 
		start[91] = 80; 
		start[93] = 51; 
	...
	void CheckLiteral() {
		switch (t.val) {
			...
			case "qualified": t.kind = 43; break;
			...
			case "or": t.kind = 59; break;
			case "and": t.kind = 60; break;
			case "&": t.kind = 62; break;
			case "<-": t.kind = 63; break;
			case "class": t.kind = 64; break;
			case "instance": t.kind = 65; break;
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 35:
				...
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 35;}
				else {t.kind = 7; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 36:
				recEnd = pos; recKind = 7;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 36;}
				else {t.kind = 7; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 37:
				recEnd = pos; recKind = 7;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 37;}
				else {t.kind = 7; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 38:
				recEnd = pos; recKind = 8;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 38;}
				else {t.kind = 8; break;}
			case 39:
				recEnd = pos; recKind = 9;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 39;}
			...
			case 55:
				...
				else if (ch == '.') {apx++; AddCh(); goto case 69;}
			...
			case 58:
				...
				if (ch >= '0' && ch <= '9') {AddCh(); goto case 9;}
				else if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 47;}
				else {t.kind = 25; break;}
			case 59:
				recEnd = pos; recKind = 7;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '=' && ch <= '@' || ch == 92 || ch == '^' || ch == '~') {AddCh(); goto case 37;}
			...
			case 60:
				recEnd = pos; recKind = 7;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch == ':' || ch >= '<' && ch <= '=' || ch >= '?' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 36;}
				else if (ch == '>') {AddCh(); goto case 44;}
				else {t.kind = 7; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 61:
				recEnd = pos; recKind = 14;
				if (ch == '!' || ch >= '%' && ch <= '&' || ch >= '*' && ch <= '+' || ch >= '-' && ch <= '/' || ch >= '<' && ch <= '@' || ch == 92 || ch == '^' || ch == '|' || ch == '~') {AddCh(); goto case 46;}
